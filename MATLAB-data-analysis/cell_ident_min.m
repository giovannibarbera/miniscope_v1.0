function [FRAMES, MIN_FRAME, MIN_FRAME_FILT,  AVG_FRAME, CNT_BAD_FRAMES, X_SHIFT, Y_SHIFT] = cell_ident_min(MacOS, use_wbar, remove_bad_frames, register_image_flag, show_movie, filePath, DARK_FRAME, N_det, width, height, first, last)
...
%% Find bad frames (row noise) and replace with previous one (old code)

% max_consecutive_pixels = 10;
...
%         for m = 1:height
%             curr_row = double(FRAMES(m,:,k));
%             aaa = diff(curr_row);
%             bbb = logical(aaa);
%             ccc = ~bbb;  
%             start1 = strfind([0,ccc==1],[0 1]);
%             end1 = strfind([ccc==1,0],[1 0]);
%             nchanges = (end1-start1) + 1;
% 
%             if max(nchanges)>max_consecutive_pixels
%                 bad_frame = 1;
%             end
%         end
        
        
        % Check for row-to-row changes
        if max(abs(diff(nansum(FRAMES(:,:,k),2)))) > 10000
            % figure; plot(abs(diff(nansum(FRAMES(:,:,k),2))));
            bad_frame = 1;

        end
        
        if squeeze(sum(sum(FRAMES(:,:,k),1),2)) == 0
            bad_frame = 1;
        end
...
if cnt_bad_frames >0
    fprintf('%d bad frames found and replaced with previous frame. Frames: ',cnt_bad_frames);
    for k = 1:cnt_bad_frames
        fprintf('%d, ',bad_frames(k));
    end
end
...
% %% Find bad frames and replace with previous one (old code)
% 
% cnt_bad_frames = 0;
% bad_frame = 0;
% bad_frames = [];
...
if show_movie
    play_movie(FRAMES, [], [], [], [], 20, 1, 0, 170,'false');
end
